...
benutzerRouter.get("/", (req: AuthedRequest, res) => {
  ...
  const benutzer = db.prepare("SELECT id, email, name, personalnr, wochenstunden, ist_admin FROM benutzer ORDER BY name").all() as any[];
...
benutzerRouter.post("/", (req: AuthedRequest, res) => {
  if (!requireAdmin(req, res)) return;
  const { name, email, personalnr } = req.body ?? {};
...
benutzerRouter.put("/:id", (req: AuthedRequest, res) => {
  ...
  db.prepare("UPDATE benutzer SET name = COALESCE(?, name), personalnr = ?, wochenstunden = COALESCE(?, wochenstunden), ist_admin = ? WHERE id = ?").run(
    name ?? null,
    personalnr ?? null,
    wochenstunden ?? null,
    istAdmin ? 1 : 0,
